<x-layout>
    ...
    <div class="bg-slate-300 w-50 h-35 mt-[17rem] border rounded flex flex-col justify-center">
        ...
        <form class="pr-10 pl-10" method="POST" action="{{ route('movie.update', ['movie' => $movie->id]) }}"
            ...
            @csrf
            <x-admin.input name="title[en]" :value=" old('title', $movie->getTranslation('title', 'en'))" :text="__('editmovie.Movie_En')" />
            <x-admin.input name="title[ka]" :value=" old('title', $movie->getTranslation('title', 'ka'))" :text="__('editmovie.Movie_Ka')" />

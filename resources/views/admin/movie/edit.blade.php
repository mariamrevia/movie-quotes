<x-layout>
    ...
    <div class="bg-slate-300 w-50 h-35 mt-[17rem] border rounded flex flex-col justify-center">
        ...
        <form class="pr-10 pl-10" method="POST" action="{{ route('movie.update', ['movie' => $movie->id]) }}"
            ...
            <div class="mt-6">
                <label class="block mb-2 uppercase font-bold text-xl text-white-700">{{__('editmovie.Movie_En')}}</label>
                <input value="{{ old('title', $movie->title) }}" name="title" id="title"
                    class="border border-gray-200 p-2 w-full rounded" />
            </div>
            <div class="mt-6">
            <label 
            class="block mb-2 uppercase font-bold text-xl text-gray-700">{{__('editmovie.Movie_Ka')}}</label>
            <input 
            name="title" 
            id="title"
            class="border border-gray-200 p-2 w-full rounded" />
